...
def cleanup_old_log_files(log_dir: Path, retention_days: int) -> int:
    ...
    for entry in log_dir.iterdir():
        if not entry.is_file():
            continue
        if not (
            entry.name.startswith("bakunawa-mech.log.")
            or entry.name.startswith("bakunawa-mech-debug.log.")
            or entry.name.startswith("bakunawa-mech.log.")
        ):
            continue
        try:
...
def _build_file_handler(config: AppConfig) -> logging.Handler:
    handler = TimedRotatingFileHandler(
        config.storage.log_dir / "bakunawa-mech.log",
        when="midnight",
        backupCount=14,
        encoding="utf-8",
    )
...
def _build_debug_file_handler(config: AppConfig) -> logging.Handler:
    handler = TimedRotatingFileHandler(
        config.storage.log_dir / "bakunawa-mech-debug.log",
        when="midnight",
        backupCount=14,
        encoding="utf-8",
    )
...
def _should_enable_debug_file(level: str) -> bool:
    return level.strip().lower() in {"debug", "trace"} or bool(os.getenv("BAKUNAWA_MECH_DEBUG_LOG"))
