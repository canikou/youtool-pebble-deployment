...
def normalize_proof_path_value(value: str) -> str:
    return value.replace("\\", "/").strip()


def proof_value_path(value: str) -> Path:
    return Path(normalize_proof_path_value(value))


def proof_value_name(value: str) -> str:
    return PurePosixPath(normalize_proof_path_value(value)).name


def canonical_receipt_proof_file_name(receipt_id: str, file_name: str) -> str:
    ...
    if not remainder:
        ...
        remainder = f"proof{suffix}" if suffix else "proof"
    return f"{receipt_id}-{remainder}"
...
def first_proof_file_name(proof_path: str | None) -> str | None:
    ...
    return proof_value_name(first) if first else None
...
def save_proof_attachment(
    ...
    stored_name = canonical_receipt_proof_file_name(receipt_id, file_name)
...
def load_embedded_payment_proofs(proof_path: str | None) -> list[ExportedPaymentProof]:
    ...
    for path in split_proof_values(proof_path):
        file_path = proof_value_path(path)
...
def materialize_imported_payment_proofs(
    ...
    if embedded:
        ...
        try:
            for proof in embedded:
                ...
                if not file_name:
                    file_name = (
                        file_name_from_url(first_proof_source_url(source_url) or "") or "proof.png"
                    )
                stored = save_proof_attachment(attachment_dir, receipt_id, file_name, bytes_data)
    ...
    existing_paths = [
        proof_value_path(path)
        for path in split_proof_values(existing_path)
        if proof_value_path(path).exists()
    ]
    if existing_paths:
